...
public class FavoriteMain extends HttpServlet {
    ...
    private static final long serialVersionUID = 1L;
        ...
        public void init() throws ServletException {
            ...
            try {
                userDAO = new UserDAO(jdbcDriverName, jdbcURL, "fav_site_user");
                favoriteDAO = new FavoriteDAO(jdbcDriverName, jdbcURL, "fav_site_favorite");
        ...
        private void addFavorite(HttpServletRequest request, HttpServletResponse response, UserBean user) 
                throws ServletException, IOException {
            List<String> errors = new ArrayList<String>();
        ...
        private void outputList(HttpServletResponse response, FavoriteForm form, List<String> messages, UserBean user) 
                throws IOException {
            FavoriteBean[] favorites;
            try {
                favorites = favoriteDAO.getUserFavorites(user.getUserId());
            } catch (MyDAOException e) {
                messages.add(e.getMessage());
                favorites = new FavoriteBean[0];
            }
            response.setContentType("text/html");
            PrintWriter out = response.getWriter();
